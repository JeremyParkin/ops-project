...
- Supported capability IDs are `schema.manage`, `automation.manage`, `records.operate`, `processes.operate`, `operations.view`, `workspace.manage_settings`, `workspace.manage_members`, and `workspace.manage_roles`.
...
- `0053_workspace_role_read_model_email_type_fix.sql`
...
- Phase 7A — Roles and Permissions Foundation: one role per membership, custom roles, membership-wide workspace reads, capability-gated interactive mutation/configuration, atomic last-administrator-safe role transitions, and a narrow Members and Roles management UI. No teams, hierarchy, row-scoped visibility, invitations, or user profiles yet.
...
- Multi-role memberships, team/manager structure, ownership, and row-scoped visibility are deliberately deferred to later Phase 7 work.
