...
- The Graph view (Process Graph 6A) is a read/inspect visual layout with structured node editing through a side panel — not a freeform canvas. There is no drag-to-move node positioning, no drag-to-connect/rewire edges, no layout persistence, and reordering remains List-only.
...
- Direct graph manipulation (6B candidate): drag-to-move node positioning, drag-to-connect/rewire edges, and layout persistence, building on the Process Graph 6A read/inspect foundation.
- Process-*run* (instance) graph visualization, distinct from the template graph view Process Graph 6A added.
