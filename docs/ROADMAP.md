...
**Status:** Complete. Phase 13.1 (Record Change History), Phase 13.2A (Workflow Execution-Log Durability), Phase 13.2B1 (Field + Choice Configuration Audit), Phase 13.2B2 (EntityType Lifecycle Audit), Phase 13.2B3a/b (Workflow and Process Template Governance Audit), Phase 13.2B4a/b (Membership, Role, Team, and Primary Manager Governance), Phase 13.2C1/C2 (People-Sensitive Access and Quality Review Configuration Governance), Phase 13.2D1 (Person Identity Governance Audit Coverage), Phase 13.2D2 (Impersonation Lifecycle Durability), the Workspace Administrative History read-policy/projection foundation, and the thin Workspace Administrative History UI are complete. Explorer implementation, broader history sources, search, export, and support tooling remain future-looking.
...
### Phase 13.2B4b1 - Team Governance Audit
...
### Phase 13.2D2 - Impersonation Lifecycle Durability

**Status:** Complete. Migration `0131` is immutable. The canonical `impersonation_started` and `impersonation_ended` `workspace_events` are now required transactional evidence for new starts, replacement ends, explicit ends, and target-deactivation cleanup. Events retain bounded `session_id` and effective-user correlation, with deterministic end reasons `explicit_end`, `replaced_by_new_session`, and `target_deactivated`; `actor_user_id` remains the real authenticated actor. Existing authority, tenancy, session replacement, effective-user behavior, raw-write closure, service-role behavior, and workspace teardown semantics remain unchanged. Dedicated lifecycle verification passed 3/3; focused backend regressions passed 131/131; focused impersonation E2E passed 1/1; static checks passed with 0 lint errors and 3 pre-existing warnings. Restricted Workspace Administrative History read-policy/projection planning is next; Explorer UI remains future-looking.

### Workspace Administrative History Read-Policy/Projection Foundation

**Status:** Complete. Migrations `0132`-`0134` are immutable. `workspace.audit.read` is an explicit capability, and `list_workspace_administrative_history_authorized` exposes only the curated, workspace-scoped allowlist of governance and identity/impersonation events through an authenticated-only RPC. The projection preserves built-in administrator and custom-role semantics, rejects active impersonation, uses bounded redacted details and truthful actor fields, and provides deterministic keyset pagination. Raw history stores remain closed. Migration `0133` corrected the helper ordering and source-column ambiguity from 0132; migration `0134` restored the audited role-update wrapper accidentally replaced by 0132 while retaining `workspace.audit.read` validation. The `/settings/history` UI is complete with authorization gating, structured Details, Category/date filters, guarded timestamps, local Load more recovery, and focused browser coverage of authorization and 53-event pagination (`2/2`). Mixed-category dogfood passed with 59 events across all six categories; record-change/runtime inclusion, export, retention, search, and support tooling remain deferred.
...
- **Record Work v1 complete:** builders can explicitly configure a metadata-defined business object as actionable work using a Workspace Member assignment field plus optional due date, status, and completion Choice options. The underlying projection remains derived from ordinary `EntityRecord` state -- no generic task/work-item table, no Process StepRun duplication -- and preserves people-sensitive visibility, workspace-timezone overdue semantics, mandatory assignment/reassignment notifications, and the archived-EntityType exclusion added by migration `0151`.
- The builder contract is now explicit: Work should be enabled only when records of the object themselves represent things a person is expected to act on. Ordinary Workspace Member relationships such as Account Manager, Executive Sponsor, or Record Owner remain plain metadata unless the builder deliberately chooses that object/field as Work. Status remains optional; due date prioritizes work but does not determine eligibility.
- Worker presentation is complete: My Work uses `Needs attention` for active/ready Process work plus eligible Record Work and `Coming later` for pending Process work only. Source/domain distinctions remain visible at the row level and in the underlying links/actions, but the page no longer duplicates Overdue/Upcoming/No-due-date sections by implementation source.
- No further Record Work expansion is currently committed. Revisit only with concrete dogfood/customer evidence. In particular, do not turn this into a second generic project-management/task subsystem: no Kanban/board, drag-and-drop, bulk reassignment, multi-assignee/multiple-assignment-field model, manager/team Record Work workload views, or due-soon/overdue Record Work notifications without a separately reviewed need.
...
- **Phase 15.1B Board pointer drag — complete:** explicit drag handles, active and optional Unset destinations, source-only archived lanes, shared compact Move/drag mutation dispatch, and optimistic movement are in place. Board V1 is shipped and remains in dogfood for touch dragging, horizontal edge auto-scroll, simultaneous real-world moves, pointer-failure focus comfort, and identity-only card sufficiency.
- **Phase 15.2 Calendar V1 — complete:** month view, URL-backed month navigation, undated and invalid-date review sections, outside-month count, crowded-day expansion, record detail links, stale-config repair, and the dogfood navigation/desktop-width refinement are in place. Calendar remains read-only in V1: no drag-to-reschedule, scheduling subsystem, persisted month, or day-detail route.
...
- **Forgotten-password recovery — complete:** a secure "Forgot password?" flow from sign-in (`/forgot-password`, `/auth/confirm`, `/reset-password`), built entirely on Supabase Auth's own supported password-reset primitives (`resetPasswordForEmail`, `verifyOtp`, `updateUser`) rather than a bespoke token system, with a strict recovery-only confirmation contract, mutation-boundary enforcement independent of the page UI, other-session revocation, and local sign-out on completion. Verified end-to-end both locally and now against the live hosted domain (`https://app.kinemahq.com/auth/confirm`) with a real mailbox-to-browser recovery. Implementation, the required Supabase dashboard configuration, and verification detail live in `PROJECT_CONTEXT.md`.
- **Hosted dogfood environment — complete:** Kinema is live at `https://app.kinemahq.com` on Vercel, connected to a dedicated persistent Supabase project separate from development/E2E, bootstrapped from a verified clean migration chain with no seed/demo data. The first persistent workspace (Household) and Jeremy's full-access administrator membership are created and verified. Hosted sign-in, workspace resolution, admin surfaces, and password recovery are all confirmed working against the real deployment. `/api/internal/process-waits` (~10 min) and `/api/internal/email-deliveries` (~2-5 min) are now invoked automatically and unattended by an external scheduler (cron-job.org), confirmed via genuine scheduled executions (not manual/test runs) in its own history. Webhook scheduling remains intentionally unconfigured -- no webhook use planned for household dogfood. Full detail in `PROJECT_CONTEXT.md`.
