...
- **v1a complete:** explicit, builder-configured record-level work semantics for an `EntityType` (Work Settings: an assignment `workspace_member` field, optional due `date` and status `choice` fields, and zero or more completion Choice options), a derived -- not materialized -- Assigned Records projection that enforces existing people-sensitive visibility and reads `workspaces.timezone` for overdue, and mandatory `record_work_assigned`/`record_work_reassigned` notifications reusing the existing notification infrastructure. An arbitrary Workspace Member field never implies work on its own; only an explicitly configured EntityType does. This milestone is backend/migrations only -- no My Work UI or builder configuration UI ships yet. Implementation and verification detail live in `PROJECT_CONTEXT.md`.
- Next: a builder-facing Work Settings configuration surface, a worker-facing Assigned Records view (most likely a distinct section inside the existing My Work page, alongside Process Step work, not merged into one undifferentiated list), and the page/action-level authorization wiring needed to make the v1a backend actually usable. Exact scope and sequencing are reviewed separately as their own slice, not assumed from this note.
- Explicit boundary, carried forward from v1a and binding for whatever ships next: Record Work makes ordinary metadata-defined business records actionable as work; it is not, and must not become, a second generic task/board subsystem competing with or duplicating Process Step Runs. No Kanban/board view, no drag-and-drop, no bulk reassignment, no new assignment model (multi-assignee fields, multiple assignment fields per object), and no due-soon/overdue notification delivery are planned as part of this direction unless a later, separately-reviewed slice finds concrete evidenced need.
