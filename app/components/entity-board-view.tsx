...
import { EntityBoardDndProvider, type BoardLaneModel } from "@/app/components/entity-board-dnd";
...
export function EntityBoardView({
  ...
  const boardLanes: BoardLaneModel[] = lanes.map((lane) => ({
    kind: lane.kind,
    id: lane.id,
    title: laneTitle(lane),
    option: lane.kind === "active" || lane.kind === "archived" ? lane.option : undefined,
    destinationValue: lane.kind === "active" ? lane.option.id : lane.kind === "unset" ? "" : undefined,
    cards: lane.records.map((record) => {
      ...
      return {
        ...
        moveAction: actionContext ? updateRecordField.bind(null, actionContext) : undefined,
      };
    }),
  }));
  ...
  return (
    <>
      ...
      <EntityBoardDndProvider
        ...
        boardFieldName={boardField.name}
        lanes={boardLanes}
      />
    </>
  );
